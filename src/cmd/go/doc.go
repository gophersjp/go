...
MAINFILEはパッケージをビルドしたときに含まれないディレクトリ内の
...
パッケージの指定についての詳しい情報は、'go help packages'を参照して下さい。

-nフラグはこれから実行するコマンドをプリントします。
-xフラグは実行中に随時コマンドをプリントします。
...
通じて足りないパッケージのチェックアウトはしますが、既存のパッケージに
ついてはアップデートのチェックをしません。

getでは'go build'および'go install'コマンドにある全てのフラグを使用して
インストールをコントロールできます。'go help build'を参照してください。

パッケージのチェックアウトやアップデートを行う際には、ローカルにインストールされた
Go言語のバージョンにマッチしたブランチやタグを探します。一番重要なルールとして、
ローカルにインストールしたバージョン"go1"で動作していれば、"go1"という
...
さらに詳しい情報は、'go help importpath'を参照してください。
...
コンパイルし、実行します。Goソースファイルは".go"のサフィックスで終わるファイルと
定義されています。
...
ことができます。詳細は'go help testfunc'を参照してください。
リストアップされたパッケージはそれぞれ個々のテストバイナリとして実行されます。

パッケージ宣言をし、"_test"サフィックスを有するテストファイルは個々のパッケージとして
...
'go test'でも利用できるものです。詳細は'go help testflag'を参照してください。

テストバイナリに対してその他任意のフラグが必要な場合は、パッケージ名の後に
指定する必要があります。go toolはマイナス記号から始まる最初の引数を
ひとつのフラグとして扱います。（マイナス記号自体は認識されません。）
その引数とその後に続くすべての引数がテストバイナリの引数として渡されます。

build flagsについてのさらに詳しい情報は、'go help build'を参照してください。
パッケージの指定についてのさらに詳しい情報は、'go help packages'を参照してください。
...
vetについてのさらに詳しい情報は、'godoc code.google.com/p/go.tools/cmd/vet'を参照してください。
パッケージの指定についてのさらに詳しい情報は、'go help packages'を参照してください。
...
使用方法については、cgoのドキュメント（godoc cmd/cgo）を参照してください。

2つ目は言語間インタフェースの汎用ツールであるSWIGプログラムです。
SWIGについての情報はhttp://swig.org/を参照してください。
...
GOPATH環境変数はGoのコードを探しにいく場所をリストアップします。
...
インポートパスや実行ファイル名を決定します。

pkg/ディレクトリはインストール済みのパッケージオブジェクトを保持します。
Go treeにしたがって、それぞれの対象オペレーティングシステムとアーキテクチャの
...
インポートパス（'go help packages'を参照してください）はローカルファイルシステムに
格納されたパッケージを示します。一般的に、インポートパスは標準パッケージ
（例えば"unicode/utf8"）かいずれかのワークスペース（'go help gopath'を
参照してください）の中から見つかったパッケージのどちらか一方を示します。
...
1つ目に、相対パスはコマンドライン上で簡易表現として用いられます。
...
パターンの文法に関する詳しい情報は、'go help packages'を参照してください。

2つ目に、Goプログラムをワークスペース外でコンパイルする場合、
...
曖昧さ回避のため、Goプログラムでは単一ワークスペース内で複数の相対パスを
用いることはできません。
...
は、特定のレポジトリを明示し、指定された名前のバージョン管理システムを
利用します。（.vcsサフィックスを含んでも含まなくても問題ありません）次いで
...
インポートパスが既知のコードホスティングサイトではなく、
さらにバージョン管理の修飾子がない場合、go toolはhttps/httpにより
インポートを取得するように試行し、ドキュメントのHTML <head>内の
<meta>タグを探します。
...
"go get"で取得するパッケージのプリフィックスとなっているか完全一致する必要があります。
完全一致でない場合は、<meta>タグの一致検証のため、プリフィックスに対してまた別の
httpリクエストが生成されます。

vcsは"git"、"hg"、"svn"などのうちの一つです。

repo-rootはスキームを含み.vcs拡張子を含まないバージョン管理システムの
ルートです。
...
新規にダウンロードしたパッケージはGOPATH環境変数に挙げられている
最初のディレクトリに書き出されます。（'go help gopath'を参照してください）

goコマンドは、利用しているGoリリースの適切なパッケージのバージョンを
ダウンロードするようにします。
...
それ以外の場合、インポートパス"P"はGOPATH環境変数に挙げられている
あるDIRに対してDIR/src/Pディレクトリの中に存在するパッケージを
表します。（'go help gopath'を参照してください。）

インポートパスが与えられていない場合、actionはカレントディレクトリの
パッケージに対して適用されます。

go toolでビルドされるパッケージに利用できないパスの予約名が以下のとおり
3つあります。
...
名前をつけることができます。詳細については'go help importpath'を参照してください。

プログラム内のいかなるパッケージでも必ず一意なインポートパスを
備えている必要があります。規則により、作成者に属する一意なプリフィックスから
...
を参照してください。pprofの--alloc_space、--alloc_objects、--show_bytes
...
という名前のテストバイナリが、'go test -c'でビルドされた後直接呼び出されます。
テストバイナリディレクトリを呼び出す際、各標準フラグ名は
-test.run=TestMyFuncや-test.vのように、'test.'をプリフィックスとして付加する
必要があります。

'go test'を実行する際、上記の一覧に記述されていないフラグは変更なしとして
扱われます。たとえば、コマンド

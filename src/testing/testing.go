...
testingパッケージは、ベンチマーク関数が十分な時間継続するようになるまでb.Nを変化させます。
出力の
...
testingパッケージは用例となるコード(Example関数)を実行し、検証します。
Example関数は結果について"Output:"で始まる一行コメントを含むことができ、テストが実行された際の関数の標準出力と比較されます。
(比較の際、前後の空白は無視されます。) Example関数の例を示します:
...
複数のExample関数を一つのパッケージ/型/関数/メソッドについて書くには、固有のサフィックスを名前に追加してください。
...
テストファイルが一つのExample関数だけを含む場合、ファイル全体が用例として提示されます。

...
ツールの終了ステータス(exit code)は、ツールの起動を間違うと"2"、問題がレポートされると"1"、"0"はそれ以外です。
ツールはすべての問題をチェックはできず、信頼性の低いヒューリスティックに依存します。
ですから、ガイダンスとしてだけ使用すべきであり、確固としたプログラムの正当性の指標として利用すべきではありません。

通常、チェックはすべて実行されますが、フラグを付けることで範囲を指定することができます。
